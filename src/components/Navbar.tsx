...
export function Navbar({ user, onLogout }: NavbarProps): JSX.Element {
  ...
  const workspaceMatch = location.pathname.match(/^\/projects\/([^/]+)\/(dashboard|onboarding)$/);
    ...
    return (
      <Link
        ...
        className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
          active ? "bg-primary-50 text-primary-700" : "text-slate-600 hover:text-slate-900 hover:bg-slate-100"
        }`}
      >
        {label}
      </Link>
    ...
    <nav className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
        <div className="flex items-center gap-6">
          <Link to="/projects" className="text-lg font-bold tracking-tight text-slate-900">
            VC<span className="text-primary-600">Reach</span>
          </Link>
